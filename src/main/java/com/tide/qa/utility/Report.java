...
public class Report extends TestBase
{
	...
	public static void createTestCaseReportHeader()
	{
        ...
        try
        {
            ...
            header+= "<style type=text/css>"+
           		 "body {"+
           		 "height:100%;"+
           		 "background: linear-gradient(rgba(65,159,248,0.92),rgba(65,159,248,0.92));"+
	...
	public static void LogInfo(String step, String Description, String Status)
	{
		 ...
		 if (Status.trim().equalsIgnoreCase("PASS"))
		 {
			 	...
			 	{
			 		String screenshotFileName = Reporter.getCurrentTestResult().getName() +"_"+step.replace(" ", "_")+"_"+counter+"Pass.jpg";
			 		ScreenShotPath = g.getResultFolderPath()+"\\Screenshots\\"+screenshotFileName.trim();
					...
					}else
					{
			 		...
			 		statusColor = "<a href=..\\"+timestamp+"\\Screenshots\\"+screenshotFileName.trim()+" target=_blank>";
			 		counter++;
			 	}
		 ...
		 {
			Fail++;
			String screenshotFileName = Reporter.getCurrentTestResult().getName() +"_"+step.replace(" ", "_")+"_"+counter+"Fail.jpg";;
			ScreenShotPath = g.getResultFolderPath()+"\\Screenshots\\"+screenshotFileName.trim();
